...
public class EquipamentoRepositoryAdapter implements EquipamentoRepositoryPort {
    ...
    // paginacao - INCOMPLETO. PRECISA IMPLMENTAR LÓGICA PARA LIDAR COM MÉTODOS QUE UTILIZAM PAG
    // SUGESTÃO: criar PageMapper com Generics. Usar a lógica do método abaixo

    // @Override
    // public Page<List<Equipamento>> findAll(Pageable pageable) {
    //     Page<List<EquipamentoJpaEntity>> pageJpa = jpaRepository.listAll(pageable);

    //     Page<List<Equipamento>> pageDomain = pageJpa.map(new Converter<List<EquipamentoJpaEntity>, List<Equipamento>>() {
    //         @Override
    //         public List<Equipamento> convert(List<EquipamentoJpaEntity> entity) {
    //             List<Equipamento> listaConvertida = new ArrayList<>();
                
    //             for (e : entity) {
    //                 listaConvertida.add(mapper.toDomain(e))
    //             }
