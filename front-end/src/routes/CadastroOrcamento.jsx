    ...
    if(orcamentoFormatado.status === "Confirmado" && account) {
      try {
            ...
            for(let i = 0; i <= 18 ; i++ ){
              ...
              if(i == 11){
                inicioFormatado += Number(cInicio + orcamentoFormatado.dataInicio[i+1]) - 3
                terminoFormatado += Number(cFim + orcamentoFormatado.dataTermino[i+1]) - 3
              }else if(i >= 13 || i < 11){
            ...
            // return console.log("Entrou com idCalendar");
             await fetch(`https://graph.microsoft.com/v1.0/me/events/${orcamentoFormatado.idCalendar}`, {
                method: 'PATCH',
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    'Content-Type': 'application/json'
